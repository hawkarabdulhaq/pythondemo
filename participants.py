...
# ───────────────────────────────────────────────────────────────
# Progress constants  (week-5 now has 4 tabs)
# ───────────────────────────────────────────────────────────────
...
:root{--bg1:#0d1117;--bg2:#161b22;--bg3:#21262d;--green:#00ff41;--cyan:#00d4ff;
--yellow:#ffcc02;--text:#f0f6fc;--text2:#8b949e;--border:#30363d;}
*{margin:0;padding:0;box-sizing:border-box;}
body{background:#0a0e13;font-family:'JetBrains Mono',monospace;color:var(--text);}
.terminal{width:95%;max-width:1000px;margin:30px auto;background:var(--bg1);
border:2px solid var(--green);border-radius:12px;padding:26px;overflow:auto;}
.header{display:flex;align-items:center;margin-bottom:22px;padding-bottom:14px;
...
.badge{display:flex;align-items:center;background:var(--bg3);padding:4px 10px;border-radius:18px;
...
.weeks{display:flex;flex-wrap:wrap;gap:12px;margin-top:4px;}
.w-item{display:flex;align-items:center;font-size:.8rem;font-weight:500;}
.w-lbl{margin-right:4px;}
.w-count{color:var(--text2);}
.no-data{text-align:center;color:var(--text2);margin:40px 0;font-style:italic;}
</style></head><body><div class="terminal">
<div class="header"><div class="title">⚡ Participants</div><div class="stats">{{STATS}}</div></div>
{{ROWS}}</div></body></html>"""

# ───────────────────────────────────────────────────────────────
# Helpers to build HTML rows
# ───────────────────────────────────────────────────────────────
def _week_row(num, done):
    req = _REQUIRED_TABS[num] or 1
    color = _WEEK_COLORS.get(num, "#00d4ff")
    return (
        f'<div class="w-item" style="color:{color};">'
        f'<span class="w-lbl">W{num}</span>'
        f'<span class="w-count">{done}/{req}</span>'
        f'</div>'
    )
...
def _rows_html(data):
    ...
    rows = []
    for p in data:
        weeks = " ".join(_week_row(k, v) for k, v in p["weeks"].items())
        rows.append(
